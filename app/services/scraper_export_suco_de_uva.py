...
def get_export_suco_uva(year: int) -> pd.DataFrame:
    URL = f"http://vitibrasil.cnpuv.embrapa.br/index.php?ano={year}&opcao=opt_06&subopcao=subopt_04" #monta url dinamica
...
def save_at_db_exportacao(df: pd.DataFrame) -> None:
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS exportacao_suco_de_uva (
...
def export_all_years_exportacao():
    ...
    for year in range(1970, 2025):
        ...
        df = get_export_suco_uva(year) #chama a function para cada ano 70's - 2024
        if not df.empty:
