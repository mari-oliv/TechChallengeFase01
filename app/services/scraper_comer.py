...
def get_proc_viniferas_all_year(year: int) -> pd.DataFrame:
    URL = f"http://vitibrasil.cnpuv.embrapa.br/index.php?ano={year}&opcao=opt_04" #monta url dinamica
...
def save_at_db_viniferas(df: pd.DataFrame) -> None:
    ...
    df.to_sql("comercializacao", conn, if_exists="append", index=False)#dataframe para sql
