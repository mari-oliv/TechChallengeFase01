...
def get_proc_uvas_mesa(year: int) -> pd.DataFrame:
    URL = f"http://vitibrasil.cnpuv.embrapa.br/index.php?ano={year}&opcao=opt_03&subopcao=subopt_03" #monta url dinamica
...
def save_at_db_viniferas(df: pd.DataFrame) -> None:
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS proc_viniferas_uvas_mesa (
    ...
    df.to_sql("proc_viniferas_uvas_mesa", conn, if_exists="append", index=False)#dataframe para sql
...
def export_all_years_viniferas():
    ...
    for year in range(1970, 2024):
        ...
        df = get_proc_uvas_mesa(year) #chama a function para cada ano 70's - 2024
        if not df.empty:
