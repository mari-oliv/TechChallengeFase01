...
def get_export_uvas_frescas(year: int) -> pd.DataFrame:
    URL = f"http://vitibrasil.cnpuv.embrapa.br/index.php?ano={year}&opcao=opt_06&subopcao=subopt_03" #monta url dinamica
...
def save_at_db_exportacao(df: pd.DataFrame) -> None:
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS exportacao_uvas_frescas (
